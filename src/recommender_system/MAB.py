"""Multi-Armed Bandit algorithms for contextual recommendation systems."""
...
def break_tie(_range: np.ndarray) -> int:
    """Break ties randomly when multiple arms have the same maximum value."""
...
def offlineEvaluate(
    mab: MAB, arms: np.ndarray, rewards: np.ndarray, contexts: np.ndarray, n_rounds: int | None = None
) -> np.ndarray:
    ...
    if n_rounds is None:  # set n_rounds to infinite number to run until all data exhausted
        n_rounds = np.inf
    elif not isinstance(n_rounds, int):
...
class EpsGreedy(MAB):
    ...
    # initialise values and raise input errors
    def __init__(self, n_arms: int, epsilon: float, Q0: float = np.inf) -> None:
    ...
    def play(self, context: np.ndarray | None = None) -> int:
        """Play a round by epsilon-greedy strategy."""
        super().play(context)
        if np.random.random_sample() <= self.epsilon:  # explore  # noqa: SIM108
...
class UCB(MAB):
    ...
    def __init__(self, n_arms: int, rho: float, Q0: float = np.inf) -> None:
        super().__init__(n_arms)
        if not rho > 0:
            raise ValueError("`rho` must be positive")
        if not (isinstance(rho, float) and np.isreal(rho)):
    ...
    def play(self, context: np.ndarray | None = None) -> int:
        """Play a round using UCB strategy."""
        super().play(context)
        self.round += 1
        self.q = np.where(
            self.clicks != 0, self.avg_rewards + np.sqrt(self.rho * np.log10(self.round) / self.clicks), self.q
        )
...
class LinUCB(MAB):
    ...
    # initialise values and raise input errors
    def __init__(self, n_arms: int, n_dims: int, alpha: float) -> None:
        if not isinstance(n_dims, int | np.integer):
    ...
    # return the best arm
    def play(self, context: np.ndarray) -> int:
        """Play a round using LinUCB strategy."""
        super().play(context)
...
class TreeBootstrap(MAB):
    ...
    # initialise values and raise input errors
    def __init__(self, n_arms: int, n_dims: int, tree: DecisionTreeClassifier | None = None) -> None:
        super().__init__(n_arms)
        if not isinstance(n_dims, int):
            raise TypeError("`n_dims` must be integer type")
        self.n_dims = n_dims
        self.tree = tree if tree is not None else DecisionTreeClassifier()
    ...
    # return the best arm
    def play(self, context: np.ndarray) -> int:
        """Play a round using Thompson sampling with decision trees."""
        super().play(context)
        for arm in range(self.n_arms):
    ...
    def update(self, arm: int, reward: float, context: np.ndarray) -> None:
        ...
        if len(self.D[0][arm]) == 0:
            ...
            self.r[0][arm] = np.vstack((np.array([0.0]), np.array([1.0])))
...
class KernelUCB(MAB):
    ...
    # initialise values and raise input errors
    def __init__(self, n_arms: int, n_dims: int, gamma: float, eta: float, kern: Callable) -> None:
        super().__init__(n_arms)
        if not isinstance(n_dims, int):
            raise TypeError("`n_dims` must be integer type")
        if not isinstance(gamma, float | np.floating):
            raise TypeError("`gamma` must be float type")
        if not np.isreal(gamma):
            raise ValueError("`gamma` must be a real number")
        if not isinstance(eta, float | np.floating):
    ...
    def play(self, context: np.ndarray) -> int:
        """Play a round using KernelUCB strategy."""
        super().play(context)
        if self.n_round == 0:  # set the initial upper bound reward
            ...
            for arm in range(self.n_arms):
                ...
                sigma = np.sqrt(
                    self.kern(shaped_context, shaped_context) - k[arm] @ self.inv_K @ k[arm]
                )  # calculate width (sigma)
